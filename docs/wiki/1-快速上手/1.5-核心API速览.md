...
本章汇总了协议层全部公开 API 类及其职责。所有类均位于 `io.github.sweetzonzi.terminal_ballistics.api` 包中——这是你在代码中唯一需要 import 的包。

## API 类全景

协议共有 10 个公开 API 类型：1 个静态入口、1 个数据 record、1 个 Builder、2 个接口、3 个枚举、1 个容器、1 个泛型键。按使用角色分类如下。

### 全部角色共用

| 类 | 类型 | 一句话职责 |
|---|------|----------|
| `TBDamageApi` | 静态工具类 | 协议唯一入口。`hurt()` 发起伤害，`hasContextFor()` 重入守卫，`getContextFor()` 获取当前上下文 |
| `TBDamageContext` | record | 一次命中的完整上下文——伤害来源、标称伤害、命中速度、命中点、命中面法线、穿深、扩展容器、handler |
| `TBDamageContextBuilder` | Builder | `TBDamageContext` 的构建器，通过 `TBDamageContext.builder()` 获取 |

### 武器侧主要使用

| 类 | 类型 | 一句话职责 |
|---|------|----------|
| `TBDamageHandler` | 接口 | 伤害发起方回调。提供 `onPenetrated` / `onBlocked` / `onRicochet` 三个主结果回调，以及由 `isOvermatch` / `isSpall` 判定触发的 `onOvermatch` / `onSpall` 派生回调。包含便捷方法 `dealDamage(target, ctx)` |
| `PenetrationResult` | 枚举 | 穿甲判定的一次性结果——`PENETRATED`（击穿）、`BLOCKED`（未击穿）、`RICOCHET`（跳弹），三者互斥 |
| `TBDamageExtensions` | 容器类 | 类型安全的扩展数据读写容器，兼有预定义 key 常量（`FUSE_DELAY`、`CALIBER`、`MASS`）。读操作总返回非 null |
| `TBDamageExtensionKey` | 泛型键 | 扩展字段的泛型 key，绑定 `ResourceLocation`、`Class<T>` 和默认值工厂。通过 `TBDamageExtensions.register()` 创建 |

### 护甲侧主要使用

| 类 | 类型 | 一句话职责 |
|---|------|----------|
| `TBHurtTarget` | 接口 | 协议伤害目标接口。声明实体自行处理穿甲判定。包含 5 个管线方法的默认实现，简单模组只需覆写 `getArmorLevel()` 和 `hurt()` |
...
- `hurt(Object target, TBDamageContext ctx)`：发起一次协议伤害。自动完成 ThreadLocal 栈的压入/弹出、穿甲管线调度、回调触发。返回实际造成的伤害量。如果目标是普通 `Entity`（未实现 `TBHurtTarget`），走原版 `entity.hurt()`。
- `hasContextFor(Object target)`：判断当前线程中是否已有针对指定目标的上下文。主要供 Mixin 注入点使用，做重入守卫。
- `getContextFor(Object target)`：获取当前线程中指定目标的命中上下文。在 `TBHurtTarget` 的管线方法（特别是 `hurt()`）内调用，以读取命中点、入射方向等信息。
...
| `handler` | `@Nullable TBDamageHandler` | 伤害发起方回调接口 |

提供两个辅助方法：`getPenetrationLevel()` 将 penetration 映射为 `ArmorLevel`；`withHandler()` 返回一个替换了 handler 的新上下文实例。
...
便捷方法：`dealDamage(target, ctx)` 等价于 `TBDamageApi.hurt(target, ctx.withHandler(this))`。

### 扩展系统

`TBDamageExtensions` 是类型安全的 key-value 存储，每个 `TBDamageContext` 持有一个实例。`set(key, value)` 写入，`get(key)` 读取——未设置时退回注册的默认值。需要复用基础扩展数据时，可以使用 `copy()` 或 `new TBDamageExtensions(other)` 创建浅拷贝。

`TBDamageExtensionKey<T>` 通过 `TBDamageExtensions.register(id, type, defaultValueFactory)` 创建全局单例，建议存为 `public static final` 常量。
...
```mermaid
graph TD
    A[TBDamageApi 入口] -->|调用| B[TBDamageContext]
    B -->|携带| C[TBDamageExtensions]
    B -->|可选携带| D[TBDamageHandler]
    C -->|持有| E[TBDamageExtensionKey 泛型键]
    A -->|目标若为 TBHurtTarget| F[TBHurtTarget]
    F -->|返回| G[ArmorLevel]
    F -->|resolvePenetration 返回| H[PenetrationResult]
    A -->|触发回调| D
```

武器侧开发者主要操作上半部分：通过 `TBDamageContext.builder()` 构造上下文，可选注入 `TBDamageHandler` 和写入扩展数据，然后调用 `TBDamageApi.hurt()`。

护甲侧开发者主要操作下半部分：让实体实现 `TBHurtTarget`，在其中返回 `ArmorLevel` 或覆写管线方法，最终由 `resolvePenetration` 返回 `PenetrationResult`。
...
- **护甲侧开发者**：进入[第三章 护甲侧开发](../3-护甲侧开发/3.1-实现TBHurtTarget.md)，深入了解 `ArmorLevel` 体系、每个管线方法的覆写策略、斜穿修正的默认实现和最终伤害计算的定制。
