...
护甲侧开发者主要操作下半部分：让实体实现 `BFHurtTarget`，在其中返回 `ArmorLevel` 或覆写管线方法，最终由 `resolvePenetration` 返回 `PenetrationResult`。
...
- **护甲侧开发者**：进入[第三章 护甲侧开发](../3-护甲侧开发/3.1-实现BFHurtTarget.md)，深入了解 `ArmorLevel` 体系、每个管线方法的覆写策略、斜穿修正的默认实现和最终伤害计算的定制。
