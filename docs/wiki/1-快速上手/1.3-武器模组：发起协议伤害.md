...
在部分场景下，你命中的实体可能只是一个**代理实体**——它本身不承担伤害，真正应该承受伤害的物理目标隐藏在代理后面。例如模块化载具的 AABB 命中后，需要将伤害路由到具体的子部件。

此时简单的三步模式（构造上下文 → 调用 `hurt`）是不够的——你需要先知道"打中的是哪个子部件"以及"是否真的打中了"。`BFDamageApi` 提供了两个便利方法供你完成这个步骤：

```java
// 假设你从原版碰撞检测获得了以下信息
EntityHitResult hitResult = ...;       // 原版碰撞检测结果
Vec3 bulletVelocity = ...;             // 弹丸速度
...
BFDamageContext ctx = BFDamageContext.builder()
    ...
    .baseDamage(35f)
    .hitVelocity(bulletVelocity)
    .hitPoint(resolved.correctedHitPoint())       // 使用修正后的精确命中点
    .hitNormal(resolved.correctedHitNormal())     // 使用修正后的精确法线
    .penetration(bulletPenetration)
...
`resolveHitTarget` 返回 null 时，表示代理实体的精确验证判定本次命中为假阳性——投射物应该继续飞行，不应被销毁。返回非 null 时，结果中包含真正的协议目标（`actualTarget`）、修正后的命中几何和解析器提供的扩展数据。

关于命中前解析的完整说明和更多用法，见[第 2.6 节](../2-武器侧开发/2.6-代理实体与命中解析.md)。
