...
本章汇总了协议层全部公开 API 类及其职责。终点弹道核心类均位于 `io.github.sweetzonzi.ballistics_framework.api` 包中；外弹道工具类位于 `io.github.sweetzonzi.ballistics_framework.api.trajectory` 子包中。

## API 类全景

协议共有 20 个公开 API 类型：终点弹道 13 个 + 外弹道 7 个。按使用角色分类如下。
...
| 类 | 类型 | 一句话职责 |
|---|------|----------|
| `BFDamageHandler` | 接口 | 伤害发起方回调。提供 11 个回调方法：5 个伤害前回调（`beforePenetrated`/`beforeBlocked`/`beforeRicochet`/`beforeOvermatch`/`beforeSpall`）和 5 个伤害后回调（`onPenetrated`/`onBlocked`/`onRicochet`/`onOvermatch`/`onSpall`），外加 1 个普通实体命中前回调（`beforeNormalEntityHit`）。包含便捷方法 `dealDamage(target, ctx)` |
| `BFHitResolver` | 接口 | 命中前目标解析。由代理实体实现，在 `BFDamageApi.hurt()` 之前调用，将 AABB 命中重定向到真正的物理伤害目标并返回精确几何 |
| `BFHitResolveResult` | record | 命中解析结果。包含真正的 `BFHurtTarget` 目标、修正后的命中点、命中面法线和解析器提供的扩展数据 |
...
- `resolveHitTarget(Entity hitEntity, Vec3 hitPoint, Vec3 delta)`：命中前目标解析。若实体实现了 `BFHitResolver`，委托其执行精确验证并返回修正后的目标与几何；否则若实体是 `BFHurtTarget`，直接包装返回。返回 null 表示应让投射物继续飞行。
- `resolveHitTarget(HitResult hitResult, Vec3 delta)`：从原版 `HitResult` 解析的便利重载。自动从 `EntityHitResult` 解包实体和命中点后委托给实体版本。非 `EntityHitResult`（如方块命中）返回 null。
...
| `afterHurt(ctx, result, finalDamage)` | 默认空实现 | 需要在伤害施加后执行后效（音效、粒子、技能触发等） |
...
| `afterHurt(wearer, slot, ctx, result, finalDamage)` | 默认空实现 | 需要在护甲被命中后执行副作用（耐久损耗、爆反消耗、碎裂降级等） |
...
武器侧实现的回调接口，所有方法均有 `default` 空实现，按需覆写。回调分为伤害前（`before*`）和伤害后（`on*`）两个阶段，在管线中于 `hurt()` 调用前后分别触发。

伤害前回调：`beforePenetrated`、`beforeBlocked`、`beforeRicochet`、`beforeOvermatch`、`beforeSpall`。适用于命中特效、撞击粒子、命中标记等应在伤害数字弹出前展示的视觉反馈。

伤害后回调：`onPenetrated`、`onBlocked`、`onRicochet`、`onOvermatch`、`onSpall`。适用于伤害统计、后效处理、破片生成等需要在伤害确认施加后执行的逻辑。

普通实体回调：`beforeNormalEntityHit(entity, ctx, baseDamage)` 和 `onNormalEntityHit(entity, ctx, baseDamage, success)`，分别在原版 `entity.hurt()` 前后触发。

判定方法：`isOvermatch`（默认：击穿且穿深 > 装甲厚度 × 1.5）、`isSpall`（默认：未击穿，或击穿但非超匹配）。协议根据这两个方法的返回值决定是否触发对应的 `before*`/`on*` 派生回调。
...
graph TD
    ...
    A -->|触发回调| D
    A -.->|命中前解析| J[BFHitResolver]
    J -->|返回| K[BFHitResolveResult]
    K -->|actualTarget 传入| A
```
...
外弹道工具位于 `io.github.sweetzonzi.ballistics_framework.api.trajectory` 子包中，提供弹道前向解算（正解）、出射方向搜索（反解）和动目标提前量预测三大能力，覆盖从"发射"到"命中"之间的物理模拟环节。

| 类 | 类型 | 一句话职责 |
|---|------|----------|
| `TrajectorySample` | record | 单个采样点：发射后时间、位置、速度 |
| `TrajectoryResult` | record | 正解结果：采样点列表 + 便捷查询方法（first/last/totalTime/terminalPoint/terminalVelocity/approximateHitNormal） |
| `FiringSolution` | record | 反解结果：出射方向、飞行时间、末速度、弹道类型（平射/高抛） |
| `BallisticConfig` | record | 拟真参数组（Cd、质量、截面积、重力、步长），含从 BFDamageExtensions 读取的工厂方法 |
| `DensityFunction` | 函数式接口 | 空气密度按位置查询，提供常值/指数衰减/真空等预设常量 |
| `MinecraftTrajectory` | 静态方法类 | MC 原版物理解算器（正解、反解、提前量），含 6 组实体预设参数常量（箭、雪球、火球等） |
| `RealisticTrajectory` | 静态方法类 | 二次阻力拟真解算器（正解、反解、提前量），基于 BallisticConfig + DensityFunction |
...
| 空气密度 | 不适用（速度乘数模型） | 每步通过 DensityFunction 查询 |
| 退化路径 | 自动识别匀速直线/纯抛物线/直线减速 | 自动识别匀速直线/纯抛物线/直线减速 |

详细说明请见[第五章 外弹道工具](../5-外弹道工具/5.1-外弹道概述.md)。

## 下一步

到这里你已经阅读了整个第一章，对协议的全貌有了基本的掌握。根据你的角色，可以选择不同的深入学习路径：

- **武器侧开发者**：进入[第二章 武器侧开发](../2-武器侧开发/2.1-构造命中上下文.md)，深入了解上下文的每个字段、命中几何信息的计算、回调的完整用法和扩展注册的最佳实践。如果你的武器系统需要处理代理实体的命中前解析，阅读 [2.6 代理实体与命中解析](../2-武器侧开发/2.6-代理实体与命中解析.md)。
- **护甲侧开发者**：进入[第三章 护甲侧开发](../3-护甲侧开发/3.1-实现TBHurtTarget.md)，深入了解 `ArmorLevel` 体系、每个管线方法的覆写策略、斜穿修正的默认实现和最终伤害计算的定制。如果是护甲物品开发者，直接看 [3.6 BFArmorMaterial 接口](../3-护甲侧开发/3.6-BFArmorMaterial接口.md)。**如果你的实体是代理实体**（如模块化载具的宿主），阅读 [3.7 代理实体：实现 BFHitResolver](../3-护甲侧开发/3.7-代理实体：实现BFHitResolver.md)。
- **想理解协议内幕**：进入[第四章 协议内幕](../4-协议内幕/4.1-穿甲判定管线.md)，了解管线的完整调用链、扩展机制的设计原理以及 ThreadLocal 上下文栈的工作方式。
- **需要弹道解算**：进入[第五章 外弹道工具](../5-外弹道工具/5.1-外弹道概述.md)，先根据弹丸物理类型选择模型，再学习正解、反解和动目标提前量。
