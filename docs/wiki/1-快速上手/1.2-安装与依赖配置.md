...
将 `ballistics_framework-1.21.1-1.0-SNAPSHOT.jar` 放入项目的 `libs/` 目录，然后在 `build.gradle` 中声明：
...
代码中只需 import `io.github.sweetzonzi.ballistics_framework.api` 包下的类。该包中的所有类和接口均为公开 API，保证跨版本兼容性。切勿 import `internal` 或 `mixin` 包下的类——它们不构成 API，行为可能在任何版本中改变。
