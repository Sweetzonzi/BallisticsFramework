...
`TBDamageContext` 的核心字段只覆盖协议中最稳定、最通用的信息：伤害来源、标称伤害、命中速度、命中点、法线和穿深。但真实弹道系统经常需要携带额外数据，例如弹体质量、口径、引信延迟、弹种、爆炸当量或自定义标签。扩展机制就是为这些"核心字段之外，但仍属于本次命中上下文"的数据准备的。
...
TerminalBallistics 使用 `TBDamageExtensionKey<T>` 作为扩展键。每个 key 同时携带三件信息：全局唯一的 `ResourceLocation`、值的 Java 类型、默认值工厂。容器的读写方法都以这个 key 为入口：
...
这样一来，编译器能在调用点约束值类型。`TBDamageExtensionKey<Float>` 只能写入 `Float`，读取时也直接得到 `Float`，调用方不需要手动强转。

## 注册一个扩展 Key

扩展 key 通过 `TBDamageExtensions.register` 注册。推荐在你的 mod API 类或常量类中声明为 `public static final`，让武器侧和护甲侧共享同一个 key 实例。

```java
public final class MyBallisticsKeys {
    public static final TBDamageExtensionKey<String> AMMO_TYPE =
    ...
    public static final TBDamageExtensionKey<Float> EXPLOSIVE_MASS =
        TBDamageExtensions.register(
...
`ResourceLocation` 的命名空间应该使用你的 mod id，路径使用语义清晰的小写名称。不要注册到 `terminal_ballistics` 命名空间下，除非这个 key 是协议本身提供的标准字段。
...
```java
float mass = ctx.extensions().get(TBDamageExtensions.MASS);
```
...
public static final TBDamageExtensionKey<AmmoTags> TAGS =
    TBDamageExtensions.register(
...
```java
TBDamageExtensions exts = new TBDamageExtensions();
exts.set(TBDamageExtensions.CALIBER, 0.125f);
exts.set(TBDamageExtensions.MASS, 9.8f);
...
| Key | 类型 | 默认值 | 含义 |
|-----|------|--------|------|
| `TBDamageExtensions.FUSE_DELAY` | `Float` | `0f` | 引信延迟，单位秒。0 表示瞬发 |
| `TBDamageExtensions.CALIBER` | `Float` | `0.1f` | 弹体口径，单位米。默认 100mm |
| `TBDamageExtensions.MASS` | `Float` | `10f` | 弹体质量，单位 kg。默认典型坦克炮穿甲弹量级 |
...
`TBDamageExtensions.register` 内部会把 key 交给 `TBExtensionKeyRegistry`。注册表按 `ResourceLocation#toString()` 保存已经注册过的 id，如果重复注册同一个 id，会抛出 `IllegalArgumentException`。
...
扩展机制提供的是调用点的泛型约束，而不是完整的运行时序列化系统。`TBDamageExtensionKey` 保存了 `Class<T> type`，但当前容器的 `set` 方法主要依赖 Java 泛型在编译期约束类型。正常使用公共 API 时，错误类型很难写进去；如果通过原始类型、反射或未经检查的强转绕过泛型，仍然可能制造运行时错误。

因此最佳实践很简单：不要使用 raw type，不要把 `TBDamageExtensionKey<?>` 随意强转为别的类型，也不要把扩展容器当作通用动态对象系统。把 key 定义成清晰的常量，围绕这些常量读写即可。
...
`TBDamageContext` 是不可变 record，但 `TBDamageExtensions` 容器本身是可变的。通常推荐在 `build()` 之前完成扩展写入，之后把它当作本次命中的只读数据使用。
...
```java
TBDamageExtensions base = new TBDamageExtensions();
base.set(MyBallisticsKeys.AMMO_TYPE, "apfsds");
base.set(TBDamageExtensions.CALIBER, 0.125f);
base.set(TBDamageExtensions.MASS, 9.8f);

TBDamageExtensions perHit = base.copy();
perHit.set(TBDamageExtensions.FUSE_DELAY, 0.05f);

TBDamageExtensions anotherHit = new TBDamageExtensions(base);
```
