...
`BFHurtInterceptor.intercept` 的判断顺序非常短：

```java
if (BFDamageApi.hasContextFor(self)) return;
if (!(self instanceof BFHurtTarget tb)) return;

BFDamageContext ctx = tb.createContextFromVanilla(source, amount);
if (ctx == null) return;

float result = BFDamageApi.hurt(self, ctx);
if (result > 0f) {
...
第二，目标不是 `BFHurtTarget`：放行。普通实体完全不受协议外伤害兼容逻辑影响。

第三，目标是 `BFHurtTarget`，并且愿意把原版伤害转换成协议上下文：调用 `createContextFromVanilla(source, amount)`，再把返回的上下文交给 `BFDamageApi.hurt`。
