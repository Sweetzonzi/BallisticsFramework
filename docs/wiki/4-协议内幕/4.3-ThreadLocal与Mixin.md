...
| `ProjectileHitResolverMixin` | `Projectile#onHit(HitResult)` | 在投射物命中处理之前做精确命中验证，自动拦截假阳性 |

前两个 Mixin 注入在 `HEAD` 并且 `cancellable = true`。它们本身不写复杂逻辑，而是统一委托给 `BFHurtInterceptor.intercept`。
...
## ProjectileHitResolverMixin：命中前自动拦截

除了 `Entity#hurt` 的 Mixin 外，协议还提供了一个位于管线之外的 Mixin——`ProjectileHitResolverMixin`——它注入到 `Projectile#onHit(HitResult)` 的 HEAD 阶段。它的作用不是接管原版伤害管线，而是在投射物进入原版命中处理之前，先做一次精确命中验证。
...
private void bf$resolveHitBeforeProcess(HitResult result, CallbackInfo ci) {
    ...
    if (resolved == null) {
        ci.cancel();  // 假阳性——投射物继续飞行
    }
}
```

步骤说明：
1. **类型筛选**：只处理 `EntityHitResult`（忽略方块命中），且目标实现了 `BFHitResolver`（忽略普通实体）
2. **搜索矢量计算**：从投射物当前 `getDeltaMovement()` 推算，取两 tick 飞行距离，钳制在安全范围内
3. **命中前解析**：调用 `BFDamageApi.resolveHitTarget(result, delta)` 让代理实体做精确验证
4. **假阳性处理**：返回 null 时 `ci.cancel()`，原版 `onHit` 不再执行——投射物留在世界中继续飞行

### 与 EntityHurtMixin 的互补

两个 Mixin 解决的是不同生命周期阶段的问题：

| Mixin | 拦截点 | 解决的问题 |
|-------|--------|-----------|
| `EntityHurtMixin` / `LivingEntityHurtMixin` | `Entity#hurt()` | 协议外伤害的穿甲判定接管（"伤害已确定发生，走谁的管线？"） |
| `ProjectileHitResolverMixin` | `Projectile#onHit()` | 投射物假阳性命中的拦截（"真的打中了还是空枪？"） |

前者在管线入口处工作，后者在管线之外工作。`ProjectileHitResolverMixin` 的 cancel 意味着投射物不被销毁且不进入伤害流程；`EntityHurtMixin` 的 cancel 发生在伤害已经进入原版 `hurt` 调用后的接管。

### 对非 BFHitResolver 实体的影响

性能影响为零——仅一次 `instanceof` 检查后直接 `return`。此 Mixin 不会对普通实体或方块命中产生任何副作用。

### 不接入 Mixin 的场景

如果武器模组自行在碰撞事件中手动调用了 `resolveHitTarget`（见 [2.6 节](../2-武器侧开发/2.6-代理实体与命中解析.md)），`ProjectileHitResolverMixin` 的自动拦截与手动调用是互补的：
- 手动调用获得更高的灵活性（自定义 delta、自定义回调处理）
- Mixin 自动拦截覆盖了未使用协议 API 的第三方投射物

两者互不冲突——Mixin 只在原版 `onHit` 被触发时生效，手动调用通常发生在原版 `onHit` 被事件系统取消之后。
