...
上下文的六个核心字段（source、baseDamage、hitVelocity、hitPoint、hitNormal、penetration）覆盖了最通用的弹道信息。但在实际的模组协作场景中，武器侧和护甲侧可能需要交换更多自定义数据——弹药类型、炸药装药量、引信延迟、特殊效应标记等。如果将这些数据全部塞入核心字段，`TBDamageContext` 的字段列表会不断膨胀，且每个模组关心的子集不同，难以统一。

扩展机制（`TBDamageExtensions` + `TBDamageExtensionKey`）正是为此设计的——它是一个类型安全的、基于泛型 key 的键值存储，挂在每个上下文实例上，允许武器侧携带任意结构化数据，护甲侧按需读取。
...
`TBDamageExtensions` 提供了两个核心方法：`set(key, value)` 和 `get(key)`。

```java
TBDamageExtensions exts = new TBDamageExtensions();

// 写入
exts.set(TBDamageExtensions.FUSE_DELAY, 0.05f);   // 引信延迟 50ms
exts.set(TBDamageExtensions.CALIBER, 0.12f);       // 120mm 口径

// 读取
float fuseDelay = exts.get(TBDamageExtensions.FUSE_DELAY);   // 0.05
float caliber   = exts.get(TBDamageExtensions.CALIBER);       // 0.12
float mass      = exts.get(TBDamageExtensions.MASS);          // 10.0（未设置，退回默认值）
```
...
base.set(TBDamageExtensions.CALIBER, 0.12f);
base.set(TBDamageExtensions.MASS, 8.5f);

TBDamageExtensions perHit = base.copy();
perHit.set(TBDamageExtensions.FUSE_DELAY, 0.05f);
```

也可以使用拷贝构造器：`new TBDamageExtensions(base)`。两种方式都会复制容器内部的 key-value 映射，但不会深拷贝扩展值对象本身。因此如果扩展值是可变对象，多个容器可能仍然共享同一个值实例；跨模组扩展字段建议使用不可变值。

## 协议预定义的扩展 Key

协议自身定义了三个标准扩展 key，均为 `Float` 类型，已注册在 `terminal_ballistics` 命名空间下：

| 常量 | 默认值 | 含义 |
|------|--------|------|
| `TBDamageExtensions.FUSE_DELAY` | `0f` | 引信延迟（秒）。0 表示瞬发引信（命中即炸）；正值表示延迟引爆（如穿甲后在内部爆炸） |
| `TBDamageExtensions.CALIBER` | `0.1f` | 弹体口径（米）。默认 0.1（100mm），典型坦克炮口径 |
| `TBDamageExtensions.MASS` | `10f` | 弹体质量（千克）。默认 10kg，典型坦克炮穿甲弹质量 |
...
如果你的模组需要携带协议预定义之外的扩展数据，可以通过 `TBDamageExtensions.register()` 注册自己的 key。注册操作建议放在模组初始化阶段（`@Mod` 构造器或静态初始化块），并保存为 `public static final` 常量，供武器侧和护甲侧共享：
...
public static final TBDamageExtensionKey<Integer> PROPELLANT_CHARGE =
    TBDamageExtensions.register(
...
public class AmmoConfig {
    public static final TBDamageExtensionKey<String> AMMO_ID = TBDamageExtensions.register(
        ResourceLocation.fromNamespaceAndPath("my_gun_mod", "ammo_id"),
        String.class, () -> "unknown"
    );
    public static final TBDamageExtensionKey<Float> EXPLOSIVE_MASS = TBDamageExtensions.register(
...
public class APFSDSBullet {
    public TBDamageContext buildContext(DamageSource source, Vec3 hitVelocity,
                                         Vec3 hitPoint, Vec3 hitNormal, float rawPen) {
        TBDamageExtensions exts = new TBDamageExtensions();
        exts.set(TBDamageExtensions.FUSE_DELAY, 0f);
        exts.set(TBDamageExtensions.CALIBER, 0.025f);
        exts.set(TBDamageExtensions.MASS, 4.5f);
