...
当一个实体实现了 `TBHurtTarget`，它不仅会响应 `TBDamageApi.hurt()` 发起的协议伤害，还会被协议的 Mixin 注入所拦截——原版的生物攻击、TNT 爆炸、投射物、魔法伤害等都可能被转换为协议伤害。`createContextFromVanilla` 方法决定了哪些原版伤害走协议管线、哪些退回原版流程。正确地实现此方法对模组的兼容性和性能都至关重要。

## Mixin 拦截的真实行为

协议通过两个 Mixin 注入类（`EntityHurtMixin` 和 `LivingEntityHurtMixin`）拦截所有的 `Entity#hurt` 调用。当一个实现了 `TBHurtTarget` 的实体被 `hurt` 时，拦截逻辑分三步走：

**第一步：重入检查。** 如果当前线程已经有针对该目标的协议上下文（即已经处于协议管线内），拦截器放行——这说明这次 `hurt` 调用是协议管线内部发出的（第五步 `target.hurt()`），不应重复拦截。

**第二步：类型检查。** 如果目标没有实现 `TBHurtTarget`，拦截器放行——普通实体完全不受协议影响。

**第三步：调用 createContextFromVanilla。** 目标实现了 `TBHurtTarget` 且不在协议管线内，拦截器调用 `tb.createContextFromVanilla(source, amount)`。如果返回 `null`，退回原版流程——伤害完全走原始路径。如果返回了一个上下文，拦截器调用 `TBDamageApi.hurt(self, ctx)` 并将 Mixin 的回调返回值设为 `true`（原版 `hurt` 返回 true，与原版行为一致）。
...
对于大多数护甲模组，最安全且最简洁的策略是在 `createContextFromVanilla` 中直接返回 `null`。这意味着**只有通过 `TBDamageApi.hurt()` 显式发起的协议伤害才走穿甲判定**，所有原版来源的伤害保持原样：
...
返回 `null` 时，协议层的存在对游戏行为没有影响。一个实现了 `TBHurtTarget` 且返回 `null` 的僵尸被另一个僵尸攻击时，伤害判定与原版完全相同。只有在枪械模组通过 `TBDamageApi.hurt()` 攻击它时，才会走穿甲管线。
...
如果你的护甲模组实现了 `createContextFromVanilla` 并对某些原版伤害进行转换，需要注意与其他也实现了 `TBHurtTarget` 的模组的交互。因为 `createContextFromVanilla` 是你类中的覆写，不会影响其他实体的行为。不同的 `TBHurtTarget` 实现者可以有不同的转换策略——一个坦克可能将爆炸转换为协议伤害，一个魔法傀儡可能连物理攻击也不转换。

但如果你的模组期望其他护甲模组也按照你的逻辑来转换原版伤害，这是做不到的——`createContextFromVanilla` 的覆写权属于每个实体类自身，不存在全局配置。如果你需要统一所有 `TBHurtTarget` 的转换行为，需要与各护甲模组作者逐个协调，或在你的文档中说明推荐策略。
