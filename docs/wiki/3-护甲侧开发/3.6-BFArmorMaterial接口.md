...
### afterHurt(wearer, slot, ctx, result, finalDamage)：护甲后效

`afterHurt` 是护甲层管线末尾的回调，在 `calculateFinalDamage` 之后、实体 `hurt()` 之前触发。此时护甲职责已结束（穿深已修正、击穿已判定、残余伤害已计算），但整体管线尚未走完，**伤害尚未施加到实体**。

这是执行护甲被命中后副作用的正确时机——武器侧的 `BFDamageHandler` 回调要等到管线完全结束后才触发，护甲侧的自处理后效应当在此处完成：
...
public void afterHurt(LivingEntity wearer, EquipmentSlot slot, BFDamageContext ctx,
    ...
    // 被击穿时额外消耗耐久（弹头穿透护甲造成结构性损伤）
    if (result == PenetrationResult.PENETRATED) {
        stack.hurtAndBreak(3, wearer, slot, eq -> {});
        // 还可根据穿深大小决定耐久损耗量
        float pen = ctx.penetration();
        if (pen > 200f) {
            stack.hurtAndBreak(2, wearer, slot, eq -> {}); // 大口径弹额外损伤
        }
    }

    // 爆反消耗：ERA 拦截化学能弹头后标记为已消耗
    if (ctx.getHandler() instanceof ChemicalEnergyWarhead
            && stack.get(MyComponents.ERA_CHARGES) > 0) {
        stack.update(MyComponents.ERA_CHARGES, 0, c -> c - 1);
    }

    // 碎裂降级：承受超过阈值的累计伤害后护甲降级
    float accumulated = stack.getOrDefault(MyComponents.ACCUMULATED_DMG, 0f);
    stack.set(MyComponents.ACCUMULATED_DMG, accumulated + finalDamage);
    if (accumulated + finalDamage > 200f && slot == EquipmentSlot.CHEST) {
        // 胸甲碎裂——替换为低级版本
        stack.shrink(1);
        wearer.setItemSlot(slot, new ItemStack(MyItems.CRACKED_CHESTPLATE));
    }
}
```

与 `BFHurtTarget.afterHurt` 的职责区分：

| 回调 | 触发时机 | 职责 |
|------|---------|------|
| `BFArmorMaterial.afterHurt` | `calculateFinalDamage` 之后、`hurt()` 之前 | 护甲物品副作用（耐久、爆反、碎裂） |
| `BFHurtTarget.afterHurt` | `hurt()` 之后 | 实体本体后效（音效、粒子、技能） |
...
第一层：BFArmorMaterial 护甲拦截
  ...
  └─ afterHurt(wearer, slot, ctx)    → 护甲后效（耐久损耗、爆反消耗等）
  ┃
